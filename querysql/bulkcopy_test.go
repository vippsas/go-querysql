...
func TestBulkcopy(t *testing.T) {
	...
	stmt, err := conn.PrepareContext(ctx, CopyIn(tableName, mssql.BulkOptions{}, columns...))
	if err != nil {
		t.Fatal(err)
	}
	defer stmt.Close()
...
func open(t testing.TB) (*sql.DB, *testLogger) {
	tl := testLogger{t: t}
	mssql.SetLogger(&tl)
	connector, err := mssql.NewConnector(makeConnStr(t).String())
	if err != nil {
...
func CopyIn(table string, options mssql.BulkOptions, columns ...string) string {
